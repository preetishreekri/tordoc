...
else if($_SESSION['mlm5_admin_type']!="Admin" && $_SESSION['mlm5_admin_type']!="Special_Employee")
...
<body>
<form name="form2" method="post" action="gen.php">
<table style="background:#FF0000; width:100%;">
<tr><td>Enter&nbsp;ID  &nbsp;&nbsp;<input type="text" name="i"  id="i" /><input type="submit" name="submit" value="Submit" /></td></tr>
</table>
</form>
...
function getToolTip($id){
	...
	while ($row = mysql_fetch_array($result, MYSQL_BOTH)) {
    	...
    	$res = "fade=[off] cssheader=[toolHeader] cssbody=[toolBody] header=[Detail For ID (आईडी का विवरण) - ".$id."] body=[".$row["m_name"]."<br />Address:<br />".$row["address"]."<br>Contact: ".$row["mobile_no"]."<br />Current Balance: ".$balance."<br>Power ID: ".$row["poweridcount"]."<br>Quantity: ".$row["qty"]."<br>Downline: ".$row["app_no"]."]";
	}
	mysql_free_result($result);
	return $res;
}
...
function getInfo($id){
	...
	$res;
	while ($row = mysql_fetch_array($result, MYSQL_BOTH)) {
    	$res = "<br />Name: ".$row["m_name"]."<br />Downline: ".$row["app_no"]."<br />Downline Power ID: ".$row["poweridcount"];
	}
	mysql_free_result($result);
	return $res;
}
