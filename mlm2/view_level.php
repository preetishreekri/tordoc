...
<?php 	
							...
							$id=$_SESSION['mlmlogin_id'];
								...
								if(!isset($result_de123['login_id']))
												{
												echo  "<tr style=\"background:Orange;\"> <td>1</td><td><a href=\"gen.php?i=$_POST[id]\">".$_POST['id']."</a></td></tr>";
												}
								else
									{
								...
								{			
										...
										$query_de = mysql_query("SELECT DISTINCT `login_id` FROM `$db2`.`$id` where `level`='$loop'");	
										...
										$query_de_1 = mysql_query("SELECT `login_id`  FROM `$db1`.`user_info` where`s_id` LIKE '$result_de[login_id]' AND `status`='0'");
											while($result_de_1=mysql_fetch_array($query_de_1))
												{
												echo  "<a href=\"gen.php?i=$result_de_1[login_id]\">".$result_de_1['login_id']."</a>, ";
												}
											}
