...
$que_id_max=mysql_query("select * from `applicant_loginid_max`");
$res_id_max=mysql_fetch_array($que_id_max);
...
if($res!=1)
{
$redirect="<meta http-equiv='refresh' content='0;index.php'/>";
echo $redirect;
exit;
}
else if ($_REQUEST["i"]<$_SESSION['mlmlogin_id'])
{
$redirect="<meta http-equiv='refresh' content='0;err.php?t=4'/>";
echo $redirect;
exit;
}
else if ($res_id_max['max']<=$_REQUEST["i"])
{
$redirect="<meta http-equiv='refresh' content='0;err.php?t=5'/>";
echo $redirect;
exit;
}
echo $redirect; ?>
...
function GenerateTree($memberid){
	...
	echo "<tr style='background:#645364; '><td colspan='5' style='text-align:left; '><a href=\"home.php\"><img src='images/home.png' style='text-align:center; '  width='45' height='45' alt='No Record' longdesc='#' /></a><a href=\"gen.php?i=$_SESSION[mlmlogin_id]\"><img src='images/1.png' style='text-align:center; '  width='45' height='45' alt='No Record' longdesc='#' /></a><a href='gen.php?i=$i2'><img src='images/back.png' style='text-align:center; '  width='45' height='45' alt='No Record' longdesc='#' /></a><a href='logout.php'><img src='images/signout.png' style='float:right'  width='100' height='40' alt='No Record' longdesc='#' /></a></td></tr>";
	...
	echo "<tr><td></td><td colspan='3' style='text-align:center; '>Login ID: ".$memberid."<br><a href=\"applicant_add.php?entry_id=$memberid\">Click here to New Entry on this ID($memberid)</a>".getInfo($memberid)."<br />"."</td><td></td></tr>";
...
function getToolTip($id){
	...
	while ($row = mysql_fetch_array($result, MYSQL_BOTH)) {
		...
		else if($row["package"]=="22")
			{
			$packageis="Royal";
			}
		else if($row["package"]=="15")
			{
			$packageis="Business";
			}
		else if($row["package"]=="7")
			{
			$packageis="Family";
			}
    	...
    	$res = "fade=[off] cssheader=[toolHeader] cssbody=[toolBody] header=[Detail For ID (आईडी का विवरण) - ".$id."] body=[".$row["m_name"]."<br />Address:<br />".$row["address"]."<br>Contact: ".$row["mobile_no"]."<br />Current Balance: ".$balance."<br>Unit Value: ".$row["poweridcount"]."<br>Quantity: ".$row["qty"]."<br>Package: ".$packageis."<br>Downline: ".$row["app_no"]."]";
	}
	mysql_free_result($result);
	return $res;
}
...
function getInfo($id){
	...
	$res;
	while ($row = mysql_fetch_array($result, MYSQL_BOTH)) {
    	$res = "<br />Name: ".$row["m_name"]."<br />Downline: ".$row["app_no"]."<br />Unit Value: ".$row["poweridcount"];
	}
	mysql_free_result($result);
	return $res;
}
