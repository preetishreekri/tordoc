...
?>
			<table class="table1" cellpadding="5" >
										...
										<?php
										if(isset($_POST['rpt'])=="" && isset($_POST['rpt1'])==""){
											    $sqlappoint=mysql_query("select * from `appointment` where `agentid`='$agentid'  order by `date`");
											    } 
											    else{
											    $sqlappoint=mysql_query("select * from `appointment` where date between '$_POST[rpt]' and '$_POST[rpt1]' and `agentid`='$agentid' order by `date`");
										...
										?>
										<tr>
										    <td><?php  echo $resappoint['date'];?></td>
											<td><?php  echo $resappoint['patient_name'];?></td>
											<td><?php  echo $resappoint['patient_reson'];?></td>
											<td><?php  echo $resappoint['patient_email'];?></td>
											<td><?php  echo $resdoct['doctor_name'];?></td>	
