 ...
 <script type="text/javascript">
		function delete_data(vals){
			...
			if(con){
			    ...
			    $.ajax({url: "delete_country.php",
			    type: 'POST',
			    data: {"countryidval": vals},
			    success: function(result){
			       
			  if (result.trim()=='NO') {
			   alert("Some states are assigned to this country so you can not delete it.");
			  }
			  if(result.trim()=='OK')
			  {
						...
						<div id="content1">
								...
								<div id="content2">
								    <?php
											  ...
											  ?>
								    <form name="myform" action="country_action.php" method="post" enctype="multipart/form-data">
									<table class="table" style="line-height:3.1;">
									    <tr>
										<td>Name of Country</td>
										<td><input type="text" name="country" value="" id="country" class="form" oninput="validateAlpha();"></td>
									    </tr>
									    <tr>
										<td>&nbsp;</td>
										<td><input type="submit" name="submit" value="insert" class="button"></td>
											...
											<table class="table1" cellpadding="5">
												...
												<?php
													...
													$sqlcity=mysql_query("select * from `country`");
													while($row=mysql_fetch_array($sqlcity))
													{
												?>
												<tr>
													<td><?php echo $row['country_name'];?></td>
													<td><a href="edit_country.php?id=<?php echo $row['id'];?>"><img src="image/edit.png"></a></td>
